...
const conf = {
  ...
  calculateNextMonthGrids(year, month) {
    ...
    const lastDayWeek = new Date(year,month-1,thisMonthDays).getDay();
    let lastEmptyGrids = [];
    if (+lastDayWeek !== 6) {
      ...
      conf.addWorkStatus(lastEmptyGrids);
      this.setData({
  ...
  calculateDays(year, month, curDate) {
    ...
    days.map(item => {
      ...
      const timestamp = new Date(item.year,item.month-1,item.day).getTime();
      if (this.config.disablePastDay && (timestamp - todayTimestamp < 0)) {
        item.disable = true;
      }
    });
    conf.addWorkStatus(days);
    const tmp = {
      'calendar.days': days,
    };
    if (curDate) {
  ...
  jumpToToday() {
    ...
    const timestamp = new Date(curYear,curMonth-1,curDate).getTime();
    const days = [{ day: curDate, year: curYear, month: curMonth}];
    conf.addWorkStatus(days);
    this.setData({
  ...
  addWorkStatus(days){
    ...
    const work = wx.getStorageSync('workList') || [];
    const night = wx.getStorageSync('nightList') || [];
    days.map( item => {
      const timestamp = new Date(item.year,item.month-1,item.day).getTime();
      if (holiday.indexOf(timestamp) >= 0) {
        item.status = 'holiday'
      } else if (work.indexOf(timestamp) >= 0) {
        item.status = 'work'
...
export const jumpToDay = (day) => {
  const self = _getCurrentPage();
  conf.jumpToDay.call(self,day);
};

export const setDayMsg = (day) => {
  const self = _getCurrentPage();
  conf.setDayMsg.call(self, day);
};
